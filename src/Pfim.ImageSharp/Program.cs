...
class Program
{
    ...
    private static void SaveAsPng(string file, IImage image, byte[] newData)
    {
        ...
        switch (image.Format)
        {
            ...
            {
                using var pixelData = IS.Image.LoadPixelData<Bgra32>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            case ImageFormat.Rgb24:
            {
                using var pixelData = IS.Image.LoadPixelData<Bgr24>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            case ImageFormat.Rgba16:
            {
                using var pixelData = IS.Image.LoadPixelData<Bgra4444>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            case ImageFormat.R5g5b5:
            {
                ...
                using var pixelData = IS.Image.LoadPixelData<Bgra5551>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            ...
            {
                using var pixelData = IS.Image.LoadPixelData<Bgr565>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            case ImageFormat.Rgb8:
            {
                using var pixelData = IS.Image.LoadPixelData<L8>(newData, image.Width, image.Height);
                pixelData.Save(fs, encoder);
                break;
            }
            default:
